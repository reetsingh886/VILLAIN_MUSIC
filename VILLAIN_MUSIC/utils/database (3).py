...
thumbnaildb = mongodb.thumbnail_status
profiledb = mongodb.ShashankProfiles
...
async def is_autoplay_enabled(chat_id: int) -> bool:
    mode = autoplaym.get(chat_id)
    if mode is not None:
        return mode
    doc = await autoplaydb.find_one({"chat_id": chat_id})
    if not doc:
        autoplaym[chat_id] = False
        return False
    autoplaym[chat_id] = True
    return True


async def enable_autoplay_db(chat_id: int):
    autoplaym[chat_id] = True
    doc = await autoplaydb.find_one({"chat_id": chat_id})
    if not doc:
...
async def disable_autoplay_db(chat_id: int):
    autoplaym[chat_id] = False
    doc = await autoplaydb.find_one({"chat_id": chat_id})
    if doc:
...
async def is_thumb_enabled(chat_id: int) -> bool:
    mode = thumbm.get(chat_id)
    if mode is not None:
        return mode
    doc = await thumbnaildb.find_one({"chat_id": chat_id})
    if not doc:
        thumbm[chat_id] = True
        return True
    status = doc.get("status", True)
    thumbm[chat_id] = status
    return status
...
async def get_user_profile(user_id: int) -> dict:
    """
    User ka profile data fetch karta hai.
    """
    profile = user_profiles.get(user_id)
    if not profile:
        profile = await profiledb.find_one({"user_id": user_id})
        if not profile:
            profile = {
                "user_id": user_id,
                "total_songs": 0,
            ...
            await profiledb.insert_one(profile)
        user_profiles[user_id] = profile
    return profile

async def update_user_stats(user_id: int, artist_name: str = "Unknown"):
    """
    User ke listening stats aur level ko update karta hai.
    """
    profile = await get_user_profile(user_id)
    new_count = profile["total_songs"] + 1

    # Simple Level Logic
    new_level = "Novice Listener"
    if new_count >= 50: new_level = "Music Lover"
    elif new_count >= 200: new_level = "Melody Master"
    elif new_count >= 500: new_level = "Sonic Legend"
    elif new_count >= 1000: new_level = "Music God"

    # Update Data
    update_data = {
        "total_songs": new_count,
        "level": new_level
    }

    # Update Artist if not unknown
    if artist_name != "Unknown":
        update_data["fav_artist"] = artist_name
    ...
    user_profiles[user_id] = await profiledb.find_one({"user_id": user_id})
